...
u32 loopLeft(u32 a, short length) {
	/*short i;
	loopLeft_label2:for (i = 0; i < length; i++) {
		a = a * 2 + a / 0x80000000;
	}
	return a;*/
...
u32 functionT(u32 a, short mode) {
	u32 b;
	b = functionB(a);
	return mode == 1 ? functionL1(b) : functionL2(b);
}
...
/*
	...
	返回值：无
*/
void iterate32(u32 X[4], u32 RK[32]) {
	short i;
	for (i = 0; i < 32; i++) {
...
int main(void) {
	...
	for (int i = 0; i < 4; i++)
	{
		memcpy(tem, (plain + i * 8), 8);
		X[i] = strtoul(tem, NULL, 16);
	}
	//scanf_s("%8x%8x%8x%8x", &X[0], &X[1], &X[2], &X[3]);
	printf("密钥：");
	for (int i = 0; i < 4; i++)
	{
		memcpy(tem, (key + i * 8), 8);
		MK[i] = strtoul(tem, NULL, 16);
	}
